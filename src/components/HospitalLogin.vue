<template>
    <div class="login-view">
        <form class="login-form mt-5">
            ...
            <h1 class="h2 mb-3 font-weight-normal d-flex justify-content-center">Hospital Sign In</h1>
    ...
    export default {
        ...
        methods: {
            attemptLogin() {
                ...
                this.blockchainHealthRecord.methods.hospitalLogin(this.form.password).send({from: this.form.userToken })
                    .then(async (receipt) => {
                        const response = receipt.events.LoginAttemptFailed || receipt.events.HospitalStaffAccount
                        if (response.returnValues.errorMessage) {
                            ...
                            const hospitalDetails = await this.blockchainHealthRecord.methods.hospitals(userDetails.hospitalId).call();

                            this.$store.dispatch('UPDATE_HOSPITAL_RECORD', hospitalDetails)
                                .then((res) => {
                                    console.log(res)
                                    this.$store.dispatch('ATTEMPT_LOGIN', userDetails)
                                        .then(() => {
                                            ...
                                            this.$router.push({ 
                                                ...
                                                params: {
                                                    hospitalId: parseInt(hospitalDetails.id, 10),
                                                    doctorId: parseInt(userDetails.id, 10)
                                                }
                                            })
    
                                        }).catch(err => { console.log('HOSPITAL LOGIN STORE ERROR:', err ); this.isLoading = false })  
                                }).catch(err => { console.log('UPDATE HOSPITAL RECORD ERROR:', err ) })  
                        }
                    }).catch((err)=>{
                        this.isLoading = false
                        console.log("HOSPITAL LOGIN API ERROR: ", err) 
                     })
            },
        ...
        async mounted() {
            this.$store.dispatch("UPDATE_LOGGED_USER_DASHBOARD", 'hospital_dashboard')
