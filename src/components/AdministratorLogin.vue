<template>
    <div class="login-view">
        <form class="login-form mt-5">
            ...
            <h1 class="h2 mb-3 font-weight-normal d-flex justify-content-center">Administrator Sign In</h1>
    ...
    export default {
        ...
        methods: {
            attemptLogin() {
                ...
                this.blockchainHealthRecord.methods.administratorLogin(this.form.password).send({from: this.form.userToken })
                    .then((receipt) => {
                        const response = receipt.events.LoginAttemptFailed || receipt.events.AdministratorAccount
                        if (response.returnValues.errorMessage) {
                            ...
                            this.$store.dispatch('ATTEMPT_LOGIN', userDetails)
                                .then(() => {
                                    ...
                                    this.isLoading = false
                                }).catch(err => { console.log('ADMINISTRATOR LOGIN STORE ERROR:', err ); this.isLoading = false })  
                        }
                    }).catch((err)=>{
                        this.isLoading = false
                        console.log("ADMINISTRATOR LOGIN API ERROR: ", err) 
                     })
            },
        ...
        async mounted() {
            this.$store.dispatch("UPDATE_LOGGED_USER_DASHBOARD", 'administrator_dashboard')
